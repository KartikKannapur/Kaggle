...
for chunk in pd.read_csv("data/train.csv", chunksize=chunksize, nrows=90000000):
     ...
     if iteration == 0:
          model_iter = xgb.train(params, xgb.DMatrix(x1, y1), 260, watchlist, maximize=True, verbose_eval=100)
          # model_iter.save_model(my_model_name)
          iteration += 1
     else:
          params.update({'process_type': 'update','updater': 'refresh','refresh_leaf': True})
          model_iter = xgb.train(params, xgb.DMatrix(x1, y1), 260, watchlist, maximize=True, verbose_eval=100, xgb_model=model_iter)
...
submission.to_csv('model_2_xbg_submission_v4.csv',index=False)